...
int main() {
	...
	//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0); // Probably dont unbind this because vertex data is bound to array, but not index data
	glBindVertexArray(0);
	...
	// repeat sampling of s and t axes
	...
	while (!glfwWindowShouldClose(window)) {
		...
		view = glm::lookAt(cameraPos,	// camera origin in world space
			cameraPos + cameraFront,	// camera direction (inverse)
			cameraUp);					// up direction for camera space
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos) {
	...
	// constrain the pitch to prevent flip
	if (pitch > 89.0f)
		pitch = 89.0f;
	if (pitch < -89.0f)
		pitch = -89.0f;

	// yaw = fmod(yaw, 360.0f); probably dumb thing, but it works :)
